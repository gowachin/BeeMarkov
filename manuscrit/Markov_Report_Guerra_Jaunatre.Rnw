...
<<transition , echo = F, eval=F, results=hide>>=
...
\begin{equation}
P_+ (Sequence) = log(P_initiale (mot_initial)) + % \sum_{i=0,\ i\neq j}^n u_{ij
% x'= ax\left(1-\frac{x}{K}\right)-bxy = 0
\end{equation}

\subsection*{test du meilleur modele}
<<transition , echo = F, eval=F, results=hide>>=
...
# # On veut des '.' au lieu des des NA
@
calculer sensi et speci pour une base
evaluer tout les sensi et speci tout au long séquence

\subsection*{markov caché et algo viterbi}
<<transition , echo = F, eval=F, results=hide>>=
...
# # On veut des '.' au lieu des des NA
@
pourquoi que c’est mieux qu’une fenetre glissante
parametrages viterbi - transition entre modèles + et -

\subsection*{smoothing (maxime)}
<<transition , echo = F, eval=F, results=hide>>=
...
Ce document est disponible en ligne sous format ``.Rnw'', contenant tout le code néccessaire à la reproduction de l'analyse, réalisée avec un script en langage R \citep{RTeam2017}, ainsi que le jeu de données de départ. L'ensemble est situé sur Github : \url{https://github.com/gowachin/BeeMarkov}
